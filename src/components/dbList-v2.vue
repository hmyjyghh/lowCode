<template>
    <div class="dblist-v2 parentboxv2" :class="'skin-'+skin+` dblist-${dataId} listDefaultFont-${getFontSizeNum(listDefaultFontSize,globalFontSize)} listActiveFont-${getFontSizeNum(listActiveFontSize,globalFontSize,true)} controlStyle-${controlStyle}`" :style="{'margin':margin,height:(height==0?300:height)+'px','background':cardTabBgColor?cardTabBgColor:titleBarBgColor,'background':titleBarBgColor?titleBarBgColor:cardTabBgColor}">
    ...
    <div class="platform_card_bts" :class="'bts-'+controlStyle">
        ...
        <div class="C00040-select" v-show="isShowSearchInput" style="margin-top: 0px;width:150px;float: right;margin-right:0px">
            ...
            <img class="serachimg" :src="dsf.url.getWebPath(`$/platform/imgs/theme/${skin}/search.png`)" alt="" @click="getDbData(activeName)" style="top: 5px;" title="搜索" srcset="">
        </div>
        ...
        <div class="img" v-if="isShowSearch" v-show="!isShowSearchInput" @click="searchIconClick()">
        ...
        <div class="img" @click="goNewFile()" v-show="activeName!='alldo'&&activeNewFile&&activeNewFile!=''">
            <i class="gzticonfont menu-icon-chuangjianx"></i>
        </div>
        <div class="img" v-show="activeName!='alldo'" @click="goPage()">
            <i class="gzticonfont menu-icon-gengduo"></i>
        </div>
        <div class="img" v-show="settingShow" @click="showCtrlSetDialogFun()">
            <i class="gzticonfont menu-icon-shezhi1"></i>
        </div>
    </div>

    <div class="video-tx" v-if="isShowVideoTx" @click="goVideoTx()">
        <img src="../../../public/platform/imgs/sdyc/videotx.png" alt="" class="default-img" title="搜索" srcset="">
        <label>视频通讯</label>
    </div>
    ...
    <el-tabs :class="getTabsCSS" v-model="activeName" @tab-click="handleClick">
        ...
        <el-tab-pane :name="item.tab" v-for="(item,index) in tabdata" :key="index">
            ...
            <div class="containbox" :style="getContainboxStyle()">
                <db-contain ref="child" 
                v-if="loadedTabName.indexOf(','+item.tab+',')>-1" 
                :listUrl="listUrl" 
                :listUrlExe="listUrlExe" 
                :skin="skin" 
                :pageSize="pageSize+2" 
                :dbinfo="item" 
                ...
                :listActiveStyle="getHoverBarStyle()" 
                :twoWhere="getTwoWhere(item.tab)" 
                :sortType="getSortType()" 
                :enableHoverBar="enableHoverBar" 
                :isShowBaoMing="isShowBaoMing" 
                :openBatchDispose="openBatchDispose" 
                :isShowNodeName="isShowNodeName" 
                :isShowName="isShowName"
                        :isShownoText="isShownoText"
                        ...
                        :actionName="{
                            ...
                            }" :showData="isShowData" :dialogHeg="dialogHeight"></db-contain>
            </div>
        </el-tab-pane>
    </el-tabs>
    ...
    <el-dialog title="视频通讯" :visible.sync="dialogVisibleVideo" width="800px" :append-to-body="true">
        <div class="iframe7-div" :style="{'height':dialogHeight+'px'}">
            <iframe scrolling="auto" allowtransparency="true" id="layui-layer-iframe7" name="layui-layer-iframe7" onload="this.className='';" class="layui-layer-iframe7" frameborder="0" 
                :src="(VideoTxurl.indexOf('DreamWeb')>-1?'':dsf.config.webRoot.webPath)+VideoTxurl"></iframe>
        </div>
    </el-dialog>
...
import tabs from '../../../public/json/tabs.json';
import {
    MessageBox,
    Message
} from 'element-ui';
export default dsf.component({
    ...
    methods: {
        ...
        CtrlSetInfoCallback(resData){
            ...
            if (resData) {
                ...
                var pdrestwo = yxArrar && yxArrar.find((v) => { return v.value == 'remote_doc_feedback' }) || undefined;
                if(pdrestwo) this.loadOuterJs(dsf.url.getWebUrl("/resource/js/public.js?v=v1.016.000.20200917_beta"));
                // 存在待签收tab则加载
                var pdres = yxArrar && yxArrar.find((v) => { return v.value == 'remote_doc_exchange' }) || undefined;
                if(pdres) this.loadOuterJs(dsf.url.getWebUrl("/resource/js/business-common.js"));
                that.selectTabData = yxArrar;
                that.loadedTabName = ",";
    ...
    mounted() {
        ...
        // this.addComponetToWindow('dblist'+dsf.uuid(), this);
        this.addComponetToWindow('dblist', this);
        //引入dreamweb-core.js
